    ...
    <section ref={sectionRef} className={styles.story} aria-label="Esplorazione tecnica del veicolo">
      <div className={styles.copy}>
        <header className={styles.intro}>
          <p>Scroll engineering · V0.4</p>
          <h2>Scorri. La macchina si apre per dettagli.</h2>
          <span>Il visual usa zoom, maschere e focus progressivo. Nessun dato 3D é simulato come se fosse CAD reale.</span>
        </header>
        ...
        {stages.map((stage, index) => (
          <article
            ...
            className={`${styles.stage} ${activeIndex === index ? styles.activeStage : ""}`}
          >
            ...
            <div>
              ...
              <div className={styles.metric}><strong>{stage.metric}</strong><small>{stage.metricLabel}</small></div>
            </div>
          </article>
        ))}
      </div>
      ...
      <div className={styles.visual} aria-hidden="true">
        <div className={styles.visualFrame}>
          <Image ref={imageRef} src={image} alt={imageAlt} fill sizes="(max-width: 900px) 100vw, 58vw" className={styles.image} priority />
          <div className={styles.vignette} />
          <div className={styles.scan} />
          <div ref={markerRef} className={styles.marker}><span /><i /></div>
          <div className={styles.visualHud}>
            <span>VALTERA / DETAIL MODE</span>
            <b>0{activeIndex + 1} / 04</b>
          </div>
          <div className={styles.activeLabel}>{stages[activeIndex]?.label}</div>
        </div>
      </div>
    </section>
  );
